...
//            }
    ...
    public function saveAction()
    {
        ...
        if ($data) {
           ...
           $productlink =  Mage::getModel('catalog/product_link')->getCollection()->getData();
         
          foreach ($productlink as $key => $values) {

               if($values['product_id'] == $product->getEntityId()){

                  $dummy[] = $values['linked_product_id'];
               }
          ...
          print_r($dummy);
         // echo('<pre>');print_r($productlink);
          //die();
           //$representing = $product->m_representing_link_data;  
           //if($dummy > 0){
          ...
          //  $_product->setMediaGallery (array('images'=>array (), 'values'=>array ())); //media gallery initialization
          //  $_product->addImageToMediaGallery('media/catalog/product/s/o/softwaredevelopment.jpg', array('image','thumbnail','small_image'), false, false);
         
           $_product->setWeight(400);
    ...
    // below code use for time format 
        ...
        // $_product->getResource()->saveAttribute($_product, 'weight');
        // $_product->getResource()->saveAttribute($_product, 'image');
        $_product->save();
            ...
            //echo ('</pre>');print_r($product->getId());die();
            try {
              //  $products->setWeight(100);
                $product->save();
